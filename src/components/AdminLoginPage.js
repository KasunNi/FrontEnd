...
const AdminLoginPage = () => {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post('http://localhost:3001/api/admin/login', credentials) // Update the API endpoint based on the actual backend routes
      .then((response) => {
		  console.log(response);
        // Handle successful login
        localStorage.setItem('token', response.data.token); // Store the token in local storage
        history.push('/admin/dashboard'); // Redirect to the customer dashboard page
      })
      .catch((error) => {
        // Handle login error
  ...
  return (
    <div className="login-container">
      ...
      <div>
        ...
        <p>Forgot your password? <Link to="/admin/reset-password">Reset Password</Link></p>
      </div>
